...
export default function MyChildrenTab({ onFamilyChanged }: MyChildrenTabProps) {
  ...
  const generateReport = async (member: FamilyMember) => {
    ...
    try {
      ...
      const parentMoon = getApproxMoonSign(profile.date_of_birth, profile.time_of_birth, profile.birth_lat, profile.birth_lon);
  ...
  return (
    <div className="space-y-4">
      ...
      {members.map(member => {
        ...
        return (
          <div key={member.id} className="glass rounded-2xl overflow-visible p-5 pb-[100px] space-y-3">
